...
function wrs_createObjectCode(object) {
	if (object.nodeType == 1) {		// ELEMENT_NODE
		var output = '<' + object.tagName.toLowerCase();
		...
		if (object.childNodes.length > 0) {
			...
			output += '</' + object.tagName.toLowerCase() + '>';
		}
...
function wrs_openEditorWindow(language, target, isIframe) {
	...
	if (!_wrs_conf_modalWindow) {
	    ...
	    wrs_createModalWindow(_wrs_int_wirisProperties['title'], iframeAttributes, _wrs_deviceProperties);
	}
}
 ...
 */
 function wrs_updateFormula(focusElement, windowTarget, mathml, wirisProperties, editMode, language) {
    if (mathml.length == 0) {
     ...
     // Add KPI measurement
    wrs_itsl_registerKpiMeasurement();
}
...
function wrs_createModalWindow(title, iframeParams, deviceProperties) {
	...
	fileref.setAttribute("href", window._wrs_conf_path + '/core/modal.css');
...
function wrs_itsl_registerKpiMeasurement() {
    var measurement;
    if (_wrs_editMode == "latex") {
        measurement = _wrs_conf_itsl_kpi_Measure_EditLatex;
    } else if (_wrs_isNewElement) {
        measurement = _wrs_conf_itsl_kpi_Measure_AddEq;
    } else {
        measurement = _wrs_conf_itsl_kpi_Measure_EditEq;
    }

    wrs_itsl_sendKpiMeasurement(measurement);
}

function wrs_itsl_sendKpiMeasurement(measurement) {
    if (!_wrs_conf_itsl_kpiUrl) {
        return;
    }

    try {
        var url = _wrs_conf_itsl_kpiUrl;
        url = url.replace('kpiMeasureSection', _wrs_conf_itsl_kpi_Section);
        url = url.replace('kpiMeasurement', measurement);
        url = url.replace('kpiMeasureContext', _wrs_conf_itsl_kpi_Context);

        var httpRequest = wrs_createHttpRequest();
        if (httpRequest) {
            var that = $(this);
            that.waitingForResponse = true;
            httpRequest.addEventListener("load", function (that) { that.waitingForResponse = false; }, false);
            httpRequest.addEventListener("error", function (that) { that.waitingForResponse = false; }, false);

            httpRequest.open('GET', url, true);
            httpRequest.withCredentials = true;
            httpRequest.send(null);

            // We need to wait, to make sure the request is not cut off when we leave the page
            // we also cannot use sync, as then we cannot set withCredentials to true
            var expire = Date.now() + 1000; // stop waiting after 1 sec
            while (that.waitingForResponse) {
                that.delay(100);
